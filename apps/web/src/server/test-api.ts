...
async function testApi() {
  ...
  try {
    ...
    if (!invalidValidationResult.success) {
      console.log(
        `   Errors: ${invalidValidationResult.error.errors.length} validation errors found`,
      )
    }
    ...
    const hasCors =
      corsHeaders['access-control-allow-origin'] && corsHeaders['access-control-allow-methods']
