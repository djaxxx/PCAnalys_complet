            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-white rounded-lg shadow p-4">
                ...
                <div className="space-y-1 text-sm text-gray-700">
                  <p>
                    <strong>Modèle:</strong> {hardware?.cpu?.name ?? 'Inconnu'}
                  </p>
                  <p>
                    <strong>Cœurs:</strong> {hardware?.cpu?.cores ?? '—'}
                  </p>
                  {hardware?.cpu?.threads !== undefined && (
                    <p>
                      <strong>Threads:</strong> {hardware.cpu.threads}
                    </p>
                  )}
                  {hardware?.cpu?.frequency !== undefined && (
                    <p>
                      <strong>Fréquence:</strong> {hardware.cpu.frequency} MHz
                    </p>
                  )}
                  {hardware?.cpu?.architecture && (
                    <p>
                      <strong>Architecture:</strong> {hardware.cpu.architecture}
                    </p>
                  )}
                </div>
              </div>
                ...
                <div className="space-y-2 text-sm text-gray-700">
                  {(hardware?.gpu ?? []).map((gpu: any, index: number) => (
                    <div key={index} className="border-b pb-2 last:border-b-0">
                      <p>
                        <strong>Modèle:</strong> {gpu.name}
                      </p>
                      {gpu.memory !== undefined && (
                        <p>
                          <strong>Mémoire:</strong> {toMB(gpu.memory)} MB
                        </p>
                      )}
                      {gpu.driver && (
                        <p>
                          <strong>Driver:</strong> {gpu.driver}
                        </p>
                      )}
                    </div>
                  ))}
              ...
              <div className="bg-white rounded-lg shadow p-4">
                ...
                <div className="space-y-1 text-sm text-gray-700">
                  <p>
                    <strong>Total:</strong> {ramTotalMB} MB
                  </p>
                  {((rawData as any)?.ram?.availableMemory || hardware?.memory?.available) && (
                    <p>
                      <strong>Disponible:</strong>{' '}
                      {toMB((rawData as any)?.ram?.availableMemory ?? hardware?.memory?.available)}{' '}
                      MB
                    </p>
                  )}
                  {(rawData as any)?.ram?.speed && (
                    <p>
                      <strong>Vitesse:</strong> {(rawData as any).ram.speed} MHz
                    </p>
                  )}
                  {(rawData as any)?.ram?.type && (
                    <p>
                      <strong>Type:</strong> {(rawData as any).ram.type}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg shadow p-4">
                ...
                <div className="space-y-2 text-sm text-gray-700">
                  {(hardware?.storage ?? []).map((storage: any, index: number) => (
                    <div key={index} className="border-b pb-2 last:border-b-0">
                      <p>
                        <strong>Nom:</strong> {storage.name}
                      </p>
                      {storage.type && (
                        <p>
                      ...
                      <p>
                        ...
                        {Math.round(
                          ((storage.capacity ?? storage.total ?? 0) as number) /
                            1000 /
                      ...
                      <p>
                        ...
                        {Math.round(
                          ((storage.freeSpace ?? storage.available ?? 0) as number) / 1000,
                        )}{' '}
                        GB
                      </p>
                    </div>
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-1 text-sm text-gray-700">
                {(() => {
                  ...
                  return (
                    <>
                      <p>
                        <strong>OS:</strong> {os?.name ?? 'Inconnu'}
                      </p>
                      <p>
                        <strong>Version:</strong> {os?.version ?? '—'}
                      </p>
                      <p>
                ...
                {rawData?.motherboard?.name && (
                  <p>
                    <strong>Carte mère:</strong> {rawData.motherboard.name}
                  </p>
                )}
                {rawData?.motherboard?.chipset && (
                  <p>
                    <strong>Chipset:</strong> {rawData.motherboard.chipset}
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow p-4 space-y-4">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Sélectionnez votre profil d'utilisation pour obtenir des recommandations
                  personnalisées
                </p>
              </div>
              ...
              {recommendations && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  ...
                  <div className="text-sm text-blue-800 whitespace-pre-wrap">{recommendations}</div>
                </div>
              )}
            </div>
          </div>
