...
vi.mock('@pcanalys/lib', () => ({
  GroqClient: vi.fn().mockImplementation(() => ({
    generateRecommendations: vi.fn().mockResolvedValue(['Mock recommendation 1', 'Mock recommendation 2']),
  })),
}));
...
describe('/api/recommend', () => {
  ...
  it('should process valid request successfully', async () => {
    ...
    await handler(req, res);
    expect((DatabaseService.getAnalysis as any)).toHaveBeenCalledWith('12345678-1234-1234-1234-123456789012');
